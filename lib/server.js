...
function BinderBuild (opts) {
  ...
  BinderBuild.super_.call(this, 'binder-build', ['build', 'registry'], opts)
...
BinderBuild.prototype._startBuild = function (api) {
  ...
  var fragments = repository.split('/tree/')

console.log('fragments', fragments)
  var repo = fragments[0]
  var branch = fragments[1]
  var force = api.params.force

  sources.fetchData(repository, function (err, data) {
    console.log(err)
    data.specified_branch = branch
    var imageName = sources.generateName(data)
    var displayName = sources.generateDisplayName(data)

console.log('weeeeooooooo', imageName, displayName)
    ...
    var startBuild = function (next) {
      ...
      self.buildInfo.findOne({ name: imageName }, function (err, existing) {
        if (err) api._badQuery({ error: err })
        if (!force && existing && existing.status === 'running') {
          api._alreadyBuilding()
    ...
    var fetchSource = function (next) {
      ...
      sources.fetchSource(data, self.apps, imageName, function (err, dir) {
    ...
    async.waterfall([
      ...
      if (err && err === 'skipping') {
        logger.info('already building {0} -- not rebuilding'.format(info.name))
        return
      } else if (err) {
