...
BinderBuild.prototype._makeBinderAPI = function () {
  return {
    ...
    fetchAll: this.fetchAllTemplates.bind(this)
  }
}
...
BinderBuild.prototype._startBuild = function (api) {
  ...
  var saveProgress = function (update) {
    return function() {
      ...
      self.buildInfo.update({ name: imageName }, info, { upsert: true }, function (err) {
        ...
        return next.apply(self, arguments.slice(-1))
      })
    }
  }
  ...
  var fetchSource = function (next) {
    self.logger.info('fetching source at {0}'.format(info.repo))
    return sources.fetchSource(repository, imageName, next)
  }
  ...
  var registerTemplate = function (imageName, dirName, next) {
    self.logger.info('registering template for {0}'.format(imageName))
    return self.registry.createTemplate(imageName, dirName, next)
  }
...
BinderBuild.prototype.fetchTemplate = function (api) {
  var self = this
...
BinderBuild.prototype.fetchAllTemplates = function (api) {
  var self = this
