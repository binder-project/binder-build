...
var makeCommands = function () {
  var commands = [
    Command('start', function (program) {
      program
        .description('Start the binder-build server')
        .option('-a, --api-key <key>', 'Binder API key')
        .option('-c, --config <file>', 'Module configuration file')
        .option('-p, --port <port>', 'the binder-build server port')
      return program
    }, function (options) {
      console.log('Starting the binder-build server...')
      var opts = {}
      if (options.config) {
    ...
    Command('stop', function (program) {
      program
        .description('Stop the binder-build server')
      return program
    }, function (options) {
      console.log('Stopping the binder-build server...')
      shell.exec('pm2 delete binder-build-start')
    })
